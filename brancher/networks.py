...
class BaseModel(torch.nn.Module):
    """
    Our base model class, which implements prenorm methods.
    """

    def prenorm_init(self):
        for module in self.modules():
            if isinstance(module, PreNormLayer):
                module.start_updates()

    def prenorm_next(self):
        for module in self.modules():
...
class GNNPolicy(BaseModel):
    ...
    def forward(self, states):
        logits = self.graph_convolution(
                states.constraint_features,
                states.edge_index,
                states.edge_attr,
                states.variable_features
        )
        return logits[states.candidates]
